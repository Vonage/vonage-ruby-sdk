...
class Vonage::Verify2Test < Vonage::Test
  ...
  def test_start_verfication_method
    ...
    stub_request(:post, uri).with(headers: headers, body: {workflow: workflow}).to_return(response)
  ...
  def test_start_verfication_method_with_opts
    ...
    stub_request(:post, uri).with(headers: headers, body: params).to_return(response)
  ...
  def test_start_verfication_method_with_workflow_that_isnt_array
    ...
    assert_raises ArgumentError do
      verify.start_verfication(workflow: workflow)
    end
  end

  def test_start_verfication_method_with_empty_workflow
    assert_raises ArgumentError do
      verify.start_verfication(workflow: [])
    end
  end
  ...
  def test_check_code_method
    ...
    stub_request(:post, check_request_uri).with(headers: headers, body: {code: code}).to_return(status: 200)
  ...
  def test_check_code_method_without_request_id
    assert_raises ArgumentError do
      verify.check_code(code: '1234')
    end
  end

  def test_check_code_method_without_code
    assert_raises ArgumentError do
      verify.check_code(request_id: request_id)
    end
  end
end
